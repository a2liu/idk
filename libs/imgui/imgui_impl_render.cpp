...
//  2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of render a
//  full-viewport to reduce likehood of issues with people using
//  VK_DYNAMIC_STATE_SCISSOR in their app without calling vkCmdSetScissor()
//  explicitly every frame. 2021-06-29: Reorganized backend to pull data from a
//  single structure to facilitate usage with multiple-contexts (all g_XXXX
//  access changed to bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory
//  validation error when buffer sizes are not multiple of
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData *draw_data,
  ...
  // to explicitely set that state. If you use VK_DYNAMIC_STATE_VIEWPORT or
...
static void
ImGui_ImplVulkan_CreateFontSampler(VkDevice device,
                                   const VkAllocationCallbacks *allocator) {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  if (bd->FontSampler)
    return;
...
static void ImGui_ImplVulkan_CreateDescriptorSetLayout(
  ...
  VkResult err = vkCreateDescriptorSetLayout(device, &info, allocator,
                                             &bd->DescriptorSetLayout);
  check_vk_result(err);
}

static void
ImGui_ImplVulkan_CreatePipelineLayout(VkDevice device,
                                      const VkAllocationCallbacks *allocator) {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  ...
  VkResult err = vkCreatePipelineLayout(device, &layout_info, allocator,
                                        &bd->PipelineLayout);
  check_vk_result(err);
}
